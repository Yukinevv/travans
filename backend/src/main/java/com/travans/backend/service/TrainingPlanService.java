...
public class TrainingPlanService {
    ...
    private TrainingDayResponse toResponse(Long userId, TrainingDay day) {
        ...
        return new TrainingDayResponse(
                ...
                matchedActivity.map(StravaActivity::getAverageSpeedMetersPerSecond).orElse(null),
                matchedActivity.map(StravaActivity::getMaxSpeedMetersPerSecond).orElse(null),
                matchedActivity.map(StravaActivity::getElevationGainMeters).orElse(null),
                matchedActivity.map(StravaActivity::getAverageHeartrateBpm).orElse(null),
                matchedActivity.map(StravaActivity::getMaxHeartrateBpm).orElse(null),
                matchedActivity.map(StravaActivity::getAverageCadenceRpm).orElse(null),
